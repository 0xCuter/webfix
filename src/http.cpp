...
bool check_route(const Request &req, M i)
{
	int count = 0;
	for (int i = 0; i < (int)req.url.length(); i++)
	{
		if (req.url[i] == '/')
			count++;
		if (count == 2)
	...
	if (count == 2)
	{
		if (req.url.substr(0, req.url.find_last_of("/")) == i->first ||
			req.url.substr(0, req.url.find_last_of("/") + 1) == i->first)
			return true;
	}
	else
	{
...
Response::Response(const Request &req): w(req.w),version(req.version), status(0), description(""),headers(""), body("") ,req_cp(req) {
    ...
    if (!method_allowed_route(req))
        ret = 405;
	else if (req.url.find("autoindex") != string::npos)
